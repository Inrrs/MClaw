...
**Anthropic 路径**：网关保留原始路径（`/anthropic/v1/messages`），bridge_fallback.py 内部做完整 Anthropic↔OpenAI 双向转换（消息、工具调用、流式 SSE）。

**OpenAI 路径**：网关只做模型映射，bridge 透传请求到 MIMO API 并注入 system prompt。
...
| `internal/manager/bridge_fallback.py` | 内置 bridge 脚本（go:embed）：Anthropic↔OpenAI 转换、system prompt 注入、流式 SSE |
...
- **Bridge 注入** (`internal/manager/inject.go`)：通过 MIMO WebSocket 执行注入（/new → 安全地址标记 → curl 下载 bridge → 等待回连），关键词检测判断 AI 响应（拒绝/确认/启动迹象）
...
- **图片处理**：bridge 自动剥离 `image_url` 内容块（MIMO API 不支持），保留文字部分
- **代理 IP 白名单** (`internal/proxy/proxy.go`)：启动时检查公网 IP 是否在代理商白名单中，不一致才更新，支持粘贴完整 URL 自动解析 uid/key
